...
本章要学习的栈和队列是时间有序表，由元素到达和离开线性结构的时间决定元素之间的关系。它们是特殊的线性表，其特殊性在于对插入和删除操作的位置进行了限制。
...
1. **栈**：限定仅在表尾（称为栈顶）进行插入和删除操作的线性表。

    - 特点：后进先出（Last In First Out, LIFO）
    - 应用：函数调用、表达式求值、括号匹配等

2. **队列**：只允许在一端（队尾）进行插入操作、另一端（队首）进行删除操作的线性表。
...
-   [第 3 章 栈和队列](#第-3-章-栈和队列)
    -   [3.1 栈](#31-栈)
        ...
        -   [3.1.2 栈的顺序存储及实现](#312-栈的顺序存储及实现)
            -   [方案一：`top`指向栈顶元素](#方案一top指向栈顶元素)
            -   [方案二：`top`指向下一个入栈位置](#方案二top指向下一个入栈位置)
        -   [共享栈](#共享栈)
            -   [共享栈的特点](#共享栈的特点)
            -   [多栈共享空间的实现](#多栈共享空间的实现)
...
顺序栈是使用数组实现的栈，需要考虑`top`和`bottom`指针的设置。有两种常见的实现方案：
...
1. 方案一更直观，`top`直接指向栈顶元素
2. 方案二更适合实现共享栈，因为可以从两端向中间增长

具体实现可参考：[SeqStack.h](../include/SeqStack.h)
...
链式队列使用链表实现，由于所有操作都在队首和队尾进行，不需要设置头节点。`front`和`rear`分别指向实际的队首和队尾节点。
...
public:
    ...
    bool deQueue(elemType& x) {
        if (front == nullptr) return false;  // 空队列

        Node* tmp = front;
        x = front->data;
        front = front->next;
        if (front == nullptr) rear = nullptr;  // 最后一个元素
        delete tmp;
        size--;
        return true;
    }

    bool getFront(elemType& x) const {
...
具体实现可参考：[LinkQueue.h](../include/LinkQueue.h)
...
由于题目给出条件$|data| \leq n$，我们可以利用这个条件设计一个高效的算法：
...
这个算法比传统的双重循环方法（时间复杂度$O(m^2)$）要高效得多。
