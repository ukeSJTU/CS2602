...
经过一些列入队出队操作后，很有可能出现front_p和rear_p都指向数组的最后一个元素，那么如果想要再入队就会超过界限，可是这个时候整个数组前面又是空的（全部元素已经出队）。这种现象叫做**假溢出**。如果简单的将所有元素向数组左端移动浪费时间，这里提出**“循环技术”**。也就是说当rear_p已经指向数组的最后一个元素后，如果再入队，那么rear_p应该移动到数组开头0，也就是说并不总是简单的+1.
