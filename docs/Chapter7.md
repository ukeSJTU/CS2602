...
- **第 1 段：** 4, 8, 12, 20, 23, 45
- **第 2 段：** 5, 10, 15

下面这张表，按照**置换选择法**(Replacement Selection)的思路，逐步展示了：

1. 如何最初将内存填满
2. 每次输出内存中的最小值后，用“待读入序列”中的下一条记录来替换刚输出的那条记录
3. 判断新读入记录是否“冻结”(即小于刚输出的值，就不能继续放在当前归并段)
4. 直到内存中所有记录都冻结，结束当前归并段，开始下一归并段

为了便于观察，下表会在“内存”一栏中用「(F)」标记已冻结的记录。若该格内没有标注 (F)，表示该记录仍在参与当前归并段。

| 步骤  | 内存 0 | 内存 1 | 内存 2 | 待读入序列                      | 输出的有序序列                                                                             |
| :---: | :----: | :----: | :----: | :------------------------------ | :----------------------------------------------------------------------------------------- |
| **0** |  NULL  |  NULL  |  NULL  | 20、4、12、8、23、5、10、45、15 | NULL                                                                                       |
| **1** |   20   |   4    |   12   | 8、23、5、10、45、15            | NULL                                                                                       |
| **2** |   20   |   8    |   12   | 23、5、10、45、15               | 4 <br/>（输出内存最小=4，读入 8 ≥ 4，未冻结）                                              |
| **3** |   20   |   23   |   12   | 5、10、45、15                   | 4、8 <br/>（输出内存最小=8，读入 23 ≥ 8，未冻结）                                          |
| **4** |   20   |   23   | 5 (F)  | 10、45、15                      | 4、8、12 <br/>（输出内存最小=12，读入 5 < 12，冻结）                                       |
| **5** | 10 (F) |   23   | 5 (F)  | 45、15                          | 4、8、12、20 <br/>（输出内存最小=20，读入 10 < 20，冻结）                                  |
| **6** | 10 (F) |   45   | 5 (F)  | 15                              | 4、8、12、20、23 <br/>（输出内存最小=23，读入 45 ≥ 23，未冻结）                            |
| **7** | 10 (F) | 15 (F) | 5 (F)  | (无)                            | 4、8、12、20、23、45 <br/>（输出内存最小=45，读入 15 < 45，冻结；至此全冻结，第 1 段结束） |
| **8** |   5    |   10   |   15   | (无)                            | 4、8、12、20、23、45； 5、10、15 <br/>（开始第 2 段，直接输出剩余冻结记录的升序）          |
| **9** |  NULL  |  NULL  |  NULL  | (无)                            | 4、8、12、20、23、45； 5、10、15 <br/>（所有数据处理完毕）                                 |
...
- **磁带 A** 上存放了第一段（Run1）\(4,\;8,\;12,\;20,\;23,\;45\)
- **磁带 B** 上存放了第二段（Run2）\(5,\;10,\;15\)
- **磁带 C** 用来存放归并后的输出结果

| 步骤 | Run1 当前值(A) | Run2 当前值(B) |           输出到 C           | 说明                                                             |
| :--: | :------------: | :------------: | :--------------------------: | :--------------------------------------------------------------- |
|  1   |       4        |       5        |              4               | 比较 4 和 5，4 更小，输出 4；Run1 指针后移，下次与 8 比较。      |
|  2   |       8        |       5        |           4，**5**           | 比较 8 和 5，5 更小，输出 5；Run2 指针后移，下次与 10 比较。     |
|  3   |       8        |       10       |          4,5，**8**          | 比较 8 和 10，8 更小，输出 8；Run1 指针后移，下次与 12 比较。    |
|  4   |       12       |       10       |        4,5,8，**10**         | 比较 12 和 10，10 更小，输出 10；Run2 指针后移，下次与 15 比较。 |
|  5   |       12       |       15       |       4,5,8,10，**12**       | 比较 12 和 15，12 更小，输出 12；Run1 指针后移，下次与 20 比较。 |
|  6   |       20       |       15       |     4,5,8,10,12，**15**      | 比较 20 和 15，15 更小，输出 15；Run2 指针后移，此时 Run2 耗尽。 |
|  7   |    20,23,45    |     — (空)     | 4,5,8,10,12,15，**20,23,45** | Run2 无剩余，直接将 Run1 剩余元素 `20,23,45` 依次输出。          |
...
请给出一组元素`18,23,47,13,5,27,54`通过逐一插入法建立大顶堆的执行过程，即写出逐一插入过程中堆的层次遍历序列。
...
当要对海量数据（如 10TB）进行排序时，通常需要采用外部排序(External Sorting)。外部排序的典型实现方法就是多路归并排序(Multi-way Merge Sort)。由于数据量过大，不能全部放入内存，所以需要将数据分块处理并在外存上进行多路归并，这本质上就是归并排序在外部存储环境下的应用。
...
删除堆顶的关键字`8`之后，将堆的最后一个关键字`12`移到堆顶，然后从堆顶开始向下调整，比较次数如下：

1. 比较`12`的左右孩子`15`和`10`，选择较小的`10`与`12`交换；
2. 比较`12`和`10`，确认`12>10`，可以将`12`向下交换；
3. `12`此时只有一个孩子`16`，比较`12`和`16`，确认`12<16`，不需要交换。
