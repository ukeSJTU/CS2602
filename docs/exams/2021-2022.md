...
B. B 和 B+树的数据记录都存储在叶结点上

C. B 树和 B+树都可以用于文件索引结构

D. B 和 B+树都能有效地支持随机检索
...
队列遵循 FIFO 先进先出原则，出队的顺序就是入队的顺序：`b d c a`。而入队的顺序就是出栈的顺序。具体的步骤如下：
...
类似书本后题目，[第二章书本后第十五题](../Chapter2.md#问题-15)。
...
第一个出栈的元素是`3`，说明此时栈里面还有`1`和`2`。但是栈顶是`2`，也就是说`p2=1`是不可能的。
...
前序遍历第一个一定是根结点，中序遍历中根结点左边的(`DBG`)是左子树，右边的(`ECF`)是右子树。
...
然后对`A`的左子树和右子树递归操作。前序遍历中`BDG`说明`B`是左子树的根结点，`DG`是`B`的左子树。中序遍历中`DG`说明`D`是左子树的根结点，`G`是左子树的右子树。
...
类似的书后习题是：[第四章书本后问题 7](../Chapter4.md#问题-7)，[第四章书本后问题 11](../Chapter4.md#问题-11)和[第四章书本后问题 12](../Chapter4.md#问题-12)。
...
1. AOE 网是无向图吗？（A 选项）
   • AOE 网 (Activity On Edge) 中，“边 (Edge)” 表示活动，箭头方向表示活动的先后次序/依赖关系，因此 必须是有向图。
   • 所以选项 A 说“AOE 网是无向图”是错误的。

2. 关键路径具有唯一性吗？（B 选项）
   • 在实际项目中，关键路径未必唯一。可能存在多条耗时相同的最长路径，都决定了项目的最早完成时间。
   • 所以“有且只有一条关键路径”这一说法并不正确。

3. 关键路径上的活动决定整体工期（C 选项）
   • 在 AOE 网中，项目的总工期是由“关键路径”决定的；一旦关键路径中某一活动延后，就会延后整个项目完成时间。
   • 这正是关键路径法 (CPM, Critical Path Method) 的核心概念。
   • 因此 C 选项是 正确 的表述。

4. 关键路径是加权最短路径吗？（D 选项）
   • 关键路径是指从项目起点到终点 耗时最长（权值和最大）的路径，这条最长路径决定了项目的最短完工时间。
   • 选项 D 说的是“加权最短路径”，与实际恰好相反。
   • 因此 D 选项是错误的。
...
A. 正确。`n`结点的树结构一定有`n-1`条边，任意添加一条边一定成环。

B. 错误。邻接矩阵的存储空间大小只和图的结点数有关系。`n`个结点的邻接矩阵就是`n*n`的矩阵，如果`i`到`j`没有，体现为`A[i][j]=noEdge`。

C. 正确。在无向图中，每条边会被两个顶点共享，导致所有顶点的度之和等于边数的两倍。因此必然是偶数。

D. 正确。图的顶点数影响顶点链表的大小，图的边数影响每个结点关联的边表的长度。
...
下面通过一个**分步表格**，演示如何使用**置换-选择法**来生成初始归并段。为方便阅读，表中用 `(F)` 标识“冻结”记录。

- **待排文件** \(FI = \{1,\;27,\;7,\;3,\;75,\;44,\;12,\;15,\;8,\;10\}\)
- **内存工作区容量** \(w = 3\)

1. 生成第一段 (Run #1)

| 步骤  | 内存 0 | 内存 1 | 内存 2 | 待读入序列                         | 输出序列（累计）    | 说明                                                                        |
| :---: | :----: | :----: | :----: | :--------------------------------- | :------------------ | :-------------------------------------------------------------------------- |
| **0** |  NULL  |  NULL  |  NULL  | 1、27、7、3、75、44、12、15、8、10 | –                   | 开始时内存为空                                                              |
| **1** |   1    |   27   |   7    | 3、75、44、12、15、8、10           | –                   | 从外存读入 3 条到内存                                                       |
| **2** |   3    |   27   |   7    | 75、44、12、15、8、10              | 1                   | 输出内存最小值 1，读入下一条数据 3 (≥1)，不冻结                             |
| **3** |   75   |   27   |   7    | 44、12、15、8、10                  | 1, 3                | 输出内存最小值 3，读入下一条 75 (≥3)，不冻结                                |
| **4** |   75   |   27   |   44   | 12、15、8、10                      | 1, 3, 7             | 输出内存最小值 7，读入下一条 44 (≥7)，不冻结                                |
| **5** |   75   |   44   | 12 (F) | 15、8、10                          | 1, 3, 7, 27         | 输出内存最小值 27，读入 12 (<27) ⇒ 冻结 (F)                                 |
| **6** |   75   | 15 (F) | 12 (F) | 8、10                              | 1, 3, 7, 27, 44     | 输出内存最小值 44，读入 15 (<44) ⇒ 冻结 (F)                                 |
| **7** | 8 (F)  | 15 (F) | 12 (F) | 10                                 | 1, 3, 7, 27, 44, 75 | 输出内存最小值 75，读入 8 (<75) ⇒ 冻结 (F) → 内存 3 元素都冻结，第 1 段结束 |

> 此时，**第 1 段**输出完毕，得到 \(\{1,\,3,\,7,\,27,\,44,\,75\}\)。  
> 内存中冻结的 3 个记录：\(\{8,\,15,\,12\}\)。  
> **还剩**最后一条未读：\(\{10\}\)。

2. 生成第二段 (Run #2)

上一段结束后，内存里有 3 个冻结的记录 \(\{8, 15, 12\}\)。现在开始**第 2 段**：

1. **解冻**内存中的 8、15、12，它们作为**新段**的起始记录。
2. 读入下一条尚未读取的记录——即 **10**。当我们从内存中输出一个最小值时，再把 10 替换进来。

|  步骤  | 内存 0 | 内存 1 | 内存 2 | 待读入序列 | 输出序列（累计）                      | 说明                                           |
| :----: | :----: | :----: | :----: | :--------: | :------------------------------------ | :--------------------------------------------- |
| **8**  |   8    |   15   |   12   |     10     | 1,3,7,27,44,75 <br/>（第 1 段结束）； | 开始第 2 段，内存的 8,15,12 解冻               |
| **9**  |   10   |   15   |   12   |     –      | (第 1 段)；**8**                      | 输出最小值 8，读入 10 (≥8) ⇒ 不冻结            |
| **10** |   10   |   15   |   –    |     –      | (第 1 段)；8，**10**                  | 输出最小值 10，暂无可读数据 => 内存只剩 15,12  |
| **11** |   15   |   –    |   –    |     –      | (第 1 段)；8,10，**12**               | 输出最小值 12（从 12、15 中比较），内存只剩 15 |
| **12** |   –    |   –    |   –    |     –      | (第 1 段)；8,10,12，**15**            | 输出剩余 15，内存清空，第 2 段结束             |

> 第 2 段输出：\(\{8,\,10,\,12,\,15\}\)。

类似的题目是：[第七章书本后问题三](../Chapter7.md#问题-3)。
...
1. 以第一个关键字为基准，也就是说`temp=17`。`{_，15，26，37，45，66，4，14}`
2. 然后从右向左找到第一个小于`temp`的元素`14`，交换位置。`{14，15，26，37，45，66，4，_}`
3. 然后从左向右找到第一个大于`temp`的元素`26`，交换位置。`{14，15，_，37，45，66，4，26}`
4. 然后从右向左找到第一个小于`temp`的元素`4`，交换位置。`{14，15，4，37，45，66，_，26}`
5. 然后从左向右找到第一个大于`temp`的元素`37`，交换位置。`{14，15，4，_，45，66，37，26}`
6. 最后将`17`插入到空的位置上，完成第一趟排序。`{14，15，4，17，45，66，37，26}`
...
| 特性     | 线性表         | 栈               | 队列             |
| -------- | -------------- | ---------------- | ---------------- |
| 访问限制 | 任意位置可访问 | 只能访问栈顶元素 | 只能访问队头元素 |
| 操作规则 | 没有特定规则   | 后进先出 (LIFO)  | 先进先出 (FIFO)  |
| 插入位置 | 任意位置插入   | 只能在栈顶插入   | 只能在队尾插入   |
| 删除位置 | 任意位置删除   | 只能在栈顶删除   | 只能在队头删除   |
...
如果树本来就空(`root==nullptr`)，直接将新结点插入为根结点。

否则，从根结点开始：

如果新结点的值等于当前结点的值，说明新结点已经存在，直接返回。

```cpp
if (x == t->data) return;
```

如果新结点的值小于当前结点的值，且当前结点的左子树为空，或者新结点的值大于当前结点的值，且当前结点的右子树为空，说明找到了插入位置，因此退出循环，准备插入数据结点。
...
(2)里面填入`x > t->data && t->right == NULL`。

如果新结点的值小于当前结点的值，就往左子树走，否则往右子树走。
...
根据上面的分析说明找到了插入位置，接下来判断是插入到左子树还是右子树。
...
1. 动态分配一个大小为`Vers`的数组，用于存储每个顶点的入度。
2. 将所有入度为 0 的顶点编号加入队列。
3. 当当前顶点指向的邻接顶点的入度减为 0 时，将该顶点加入队列。
