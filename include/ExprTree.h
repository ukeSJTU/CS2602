...
class ExprTree : public BTree<elemType>
{
   public:
...
// 0: op1优先级与op2相同
template <class elemType>
int ExprTree<elemType>::comparePriority(const char op1, const char op2)
{
    switch (op1) {
        case '(':
            return 1;
...
// 计算表达式树的值
template <class elemType>
int ExprTree<elemType>::calcExprTree()
{
    ...
    while (!nodeStack.isEmpty()) {  // 遍历树
        ...
        switch (flag) {
            case 2:  // 处理当前节点的操作
                nodeStack.pop();
                if ((this->getData(node) >= '0') &&
                    (this->getData(node) <= '9')) {   // 当前节点是数字
